...
const { errores, form, handleUpdate } = useDeliverables();
...
onMounted(async () => {
  ...
  const response = await deliverablesService.show(projectId, id);
  if (response.status && response.items) {
    form.value = response.items;
  } else {
    snackbar.value = { show: true, text: 'Entregable no encontrado', color: 'error' };
    router.push({ name: 'deliverables', params: { projectId } });
  }
  loader.value = false;
});
</script>
  ...
  <VRow>
        ...
        <VCardItem class="pb-4">
          <VCardTitle>
            <h4 class="text-h4 text-wrap">
              Editar <strong>Entregable</strong>
            </h4>
          </VCardTitle>
        </VCardItem>
      </VCard>
    ...
    <VCol cols="12">
      <VForm @submit.prevent="requestSave(handleUpdate)">
        <DeliverableForm :form="form" :errores="errores" :project-id="Number(route.params.projectId)" />
      </VForm>
    </VCol>
