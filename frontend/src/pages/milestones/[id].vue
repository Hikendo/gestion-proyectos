...
const { errores, form, handleUpdate } = useMilestones();
...
onMounted(async () => {
    ...
    const response = await milestonesService.show(projectId, id);
    if (response.status && response.items) {
        form.value = response.items;
    } else {
        snackbar.value = { show: true, text: 'Hito no encontrado', color: 'error' };
        router.push({ name: 'milestones', params: { projectId } });
    }
    loader.value = false;
});
</script>
