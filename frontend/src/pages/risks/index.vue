          ...
          >
            ...
            <VCardText class="pa-2">
              <div class="d-flex flex-column gap-2">
                ...
                <!-- Tarjeta de riesgo -->
                <VCard
                  v-for="risk in getByStatus(col.id)"
                  :key="risk.id"
                  variant="outlined"
                  class="risk-card"
                  :class="{ 'risk-dragging': draggedRisk?.id === risk.id }"
                  draggable="true"
                  @dragstart="onDragStart($event, risk)"
                  @dragend="onDragEnd"
                >
...
.risk-card:active { cursor: grabbing; }

.risk-dragging { opacity: 0.4; }
