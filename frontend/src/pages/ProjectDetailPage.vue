...
<template>
  <section class="page-grid">
    <FeaturePanel :title="projectLabel" description="El ID del proyecto llega desde la ruta y las secciones del detalle se separan en subrutas hijas.">
      <template #actions>
        <RouterLink :to="{ name: 'projects' }" class="button primary">Volver al listado</RouterLink>
      </template>

      <p class="feature-copy">
        Ruta actual tipada con el parametro <strong>{{ projectId }}</strong>. Las areas de planning, execution y delivery ya no viven en una sola vista larga.
      </p>
      ...
      <nav class="project-tabs">
        <RouterLink v-for="tab in tabs" :key="tab.name" :to="{ name: tab.name, params: { projectId } }" class="project-tabs__link">
          {{ tab.label }}
        </RouterLink>
      </nav>
